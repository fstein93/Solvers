...
vector<size_t> Square_Grid::elements_in_same_row_or_col_as(const size_t global) const noexcept
{
  vector<size_t> indices(elements_in_same_row_as(global)) ;
  const size_t first_element = first_element_in_col_of(global) ;
  for (size_t idx = first_element ; idx < global ; idx+=number_of_cols_) indices.push_back(idx) ;
  for (size_t idx = global+number_of_cols_ ; idx < first_element+number_of_fields_ ; idx+=number_of_cols_) indices.push_back(idx) ;
  return indices ;
}
