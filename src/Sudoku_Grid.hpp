...
public:
  ...
  constexpr T global2rowblock(const T global) const noexcept {return Sudoku_Grid<T>::global2row(global)/3 ;} ;
  constexpr T global2colblock(const T global) const noexcept {return Sudoku_Grid<T>::global2col(global)/3 ;} ;
  ...
  std::vector<T> elements_in_same_block_as(const T global) const noexcept 
  {
    ...
    indices.reserve(Sudoku_Grid<T>::number_of_cols_) ;
    for (T idx_first_col = first_element ; idx_first_col < first_element+number_of_rows_per_block_*Sudoku_Grid<T>::number_of_cols_ ; idx_first_col += Sudoku_Grid<T>::number_of_cols_)
    {
  ...
  constexpr T first_element_in_block_of(const T global) const noexcept {return number_of_rows_per_block_*Sudoku_Grid<T>::number_of_cols_*global2rowblock(global)+number_of_cols_per_block_*global2colblock(global) ;} ;
  ...
  {
    std::vector<T> indices(Square_Grid<T>::elements_in_same_row_or_col_as(global)) ;
    const T first_element = first_element_in_block_of(global) ;
    for (const T& row : left_over_indices(Sudoku_Grid<T>::global2row(global)%3, number_of_rows_per_block_))
    {
      for (const T& col : left_over_indices(Sudoku_Grid<T>::global2col(global)%3, number_of_cols_per_block_))
      {
        indices.push_back(first_element + row*Sudoku_Grid<T>::number_of_cols_ + col) ;
      }
    }
    return indices ;
  };
